...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for tool in &tools.tools {
        ...
        println!("  description: {}", tool.description.as_ref().map(|d| d.as_ref()).unwrap_or("None"));
        println!("  input_schema: {{");
        let schema = &tool.input_schema;
        println!("    type: {}", schema.get("type").map(|v| v.to_string()).unwrap_or_else(|| "null".to_string()));
        println!("    required: {}", schema.get("required").map(|v| v.to_string()).unwrap_or_else(|| "null".to_string()));
        ...
        println!("    additionalProperties: {}", schema.get("additionalProperties").map(|v| v.to_string()).unwrap_or_else(|| "null".to_string()));
        println!("  }}");
        println!("}}");
        println!();
    }
